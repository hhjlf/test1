...
public boolean idCheck(String id) {
	...
	try {
		...
		if(!rs.next()) result =false; //중복된아이디없으면 펄스
...
public StudentVO getMember(String id) {
	...
	try {
		...
		if(rs.next()) {
			...
			vo.setAddress1(rs.getString("adress1"));
...
public void updateMember(StudentVO vo) {
	...
	try {
		conn = getConnection();
		pstmt = conn.prepareStatement("update student set pass=?,phone1=?,phone2=?,phone3=?,email=?,zipcode=?,adress1=?,address2=? where id=?");
